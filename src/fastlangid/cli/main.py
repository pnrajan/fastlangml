"""Command-line interface for fastlangid.

Usage:
    fastlangid "bonjour"              # Detect language, output JSON
    fastlangid "hello" --top-k 3      # Get top 3 languages
    fastlangid --version              # Show version
    fastlangid batch input.txt        # Batch process file
    fastlangid bench --dataset self   # Run benchmark
    fastlangid backends               # List backends
"""
...
ISO3_TO_ISO1 = {
    ...
    "azb": "az", "tuk": "tk", "crh": "crh", "tyv": "tyv", "sah": "sah",
...
def run_wili_benchmark(detector: Any, args: argparse.Namespace) -> int:
    ...
    # Load test data
    with open(data_dir / "x_test.txt", "r", encoding="utf-8") as f:
        texts = [line.strip() for line in f]

    with open(data_dir / "y_test.txt", "r", encoding="utf-8") as f:
...
def backends_command(args: argparse.Namespace) -> int:
    ...
    from fastlangid import get_available_backends
    from fastlangid.backends import BACKEND_RELIABILITY
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--version", "-V",
        action="version",
        version=f"fastlangid {get_version()}",
    )
    ...
    bench_parser.add_argument("--n-samples", "-n", type=int, default=1000, help="Number of samples to benchmark")
    bench_parser.add_argument("--languages", "-l", type=str, help="Filter to specific languages (comma-separated, e.g., 'en,es,fr')")
    ...
    if argv and argv[0] not in ["detect", "batch", "bench", "backends", "-h", "--help", "-V", "--version"]:
